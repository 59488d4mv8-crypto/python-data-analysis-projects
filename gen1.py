...
U.append({"unitId": "u01", "title": "Python 基础", "chapters": [
    chapter("ch0101", "变量与数据类型",
        ...
        "Python 是商务数据分析的主流语言。变量用于存储销售金额、产品名称等数据。本章从 print 函数开始介绍整数、浮点数、字符串、布尔值四种核心类型。",
        [
            cs("cs010101", "print 与变量", "print('Hello')\nprice=2999.99\nprint('单价:'+str(price))", "print 输出内容。字符串拼接需要 str() 转换数字。"),
            cs("cs010102", "类型转换", "s='58000'\nnum=float(s)\nprint(num*1.1)", "CSV 数据常为字符串，float/int/str 用于类型转换。"),
            cs("cs010103", "算术运算", "p,d,q=1599,0.2,230\nprint(p*(1-d)*q)", "+-*/ 用于计算折扣与总销售额。")
        ],
        [
            ex("ex010101", "定义 name='张三' company='ABC科技' years=3 三个变量并打印拼接介绍。",
                ...
                ["字符串用双引号", "拼接用 +，数字需 str() 转换"], "name='张三'\ncompany='ABC'\nyears=3\nprint(name+company+str(years))"),
    ...
    chapter("ch0102", "列表与字典",
        ...
        "列表用于存储有序同类数据（如月销售额序列），字典用于存储带属性的业务对象（如产品的名称、价格、销量）。",
        [
            cs("cs010201", "列表基础", "p=['笔记本','平板']\ns=[125,280]\nprint(p[0],s[-1])", "列表用 [] 创建。索引从 0 开始，-1 指最后一个元素。"),
            cs("cs010202", "切片与 append", "q=[32000,28500,41200,35800]\nprint(q[:2])\nq.append(49100)\nprint(len(q))", "list[start:end] 切片。append 添加元素。len 获取长度。"),
            cs("cs010203", "字典操作", "p={'name':'手机','price':2999}\np['stock']=350\np['price']=2799\nprint(p)", "字典用 {} 创建。dict[key]=value 可新增或修改键值对。")
        ],
        [
            ...
            ex("ex010202", "创建 customer 字典：name='李晓明',age=35,city='上海',is_vip=True,total_purchase=28600。修改 total_purchase 为 35000，新增 last_order='2025-05-20'。",
                ...
                ["key:value 格式", "布尔值首字母大写", "赋值语句修改/新增"],
        ...
        ], "列表适合有序同类数据，字典适合带属性的业务对象。"),
    chapter("ch0103", "函数与控制流",
        ["掌握 def 函数与 return", "掌握 if-elif-else", "掌握 for 循环"],
        "函数封装可复用逻辑，条件判断处理业务规则，for 循环批量处理数据。",
        [
            cs("cs010301", "函数定义", "def profit(r,c):\n    return r-c,(r-c)/r*100\np,rt=profit(58000,36200)\nprint(p,round(rt,1))", "def 定义函数，可 return 多个值。round(x,n) 保留 n 位小数。"),
            cs("cs010302", "条件判断", "s=18500\nif s>=30000:l='钻石'\nelif s>=15000:l='金卡'\nelif s>=5000:l='银卡'\nelse:l='普通'\nprint(l)", "条件从高到低排列。每块冒号结尾后缩进 4 空格。"),
            cs("cs010303", "for 循环", "t=0\nfor x in [12500,8900,15600]:\n    t=t+x\nprint(t)", "for 遍历列表每项，循环内可累加汇总。")
        ],
        [
            ...
            ex("ex010302", "q=[120,85,156,78,203,145]，up=[299,1599,899,499,2599,1299]。用 for+索引 计算每产品销售额存入 product_revenues，总销售额 total_revenue。",
                ...
                ["range(len(q)) 获取索引 i", "q[i]*up[i] append 到 product_revenues", "total_revenue 累加"],
                "for i in range(len(q)):\n    r=q[i]*up[i]\n    product_revenues.append(r)\n    total_revenue=total_revenue+r")
        ], "函数、条件、循环是编程三要素。")
]})

print("U1 章数:", len(U[0]["chapters"]))
with open("assets/data/u1.json", "w", encoding="utf-8") as f:
